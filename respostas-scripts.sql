...
-- ETL
-- removendo duplicatas na tabela dept_locations
create table distinct_dept_locations as 
select Dnumber, 
	   MIN(Dlocation) as Dlocation
from dept_locations
group by Dnumber;
...
from distinct_dept_locations;

drop table distinct_dept_locations;
...
-- 9.	Mesclar consultas employee e departament para criar uma tabela employee com 
-- 		o nome dos departamentos associados aos colaboradores. A mescla terá como base 
-- 		a tabela employee. Fique atento, essa informação influencia no tipo de junção
select e.*,
...
-- 		Isso pode ser feito com consulta SQL ou pela mescla de tabelas com 
-- 		Power BI. Caso utilize SQL, especifique no README a query utilizada no processo.
select e.Fname,
	   e.Lname,
	   concat(e2.Fname, ' ', e2.Lname) as Manager
from employee e
...
-- 12.	Mescle as colunas de Nome e Sobrenome para ter apenas uma coluna definindo 
...
-- 		departamento-local seja único. Isso irá auxiliar na criação do modelo estrela 
-- 		em um módulo futuro.
select d.*,
...
-- 14.	Explique por que, neste caso supracitado, podemos apenas utilizar o mesclar 
-- 		e não o atribuir. 
-- [RESPOSTA]: Para acrescentar colunas é necessario que as tabelas que serão empilhadas tenham 
-- os mesmos cabeçalhos, na mesma ordem.
